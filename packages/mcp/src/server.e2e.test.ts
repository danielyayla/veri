...
async function rpcSession(requests: object[], expectIds: number[]): Promise<Map<number, RpcResponse>> {
  const child = spawn(process.execPath, [SERVER, FIXTURE], { stdio: ['pipe', 'pipe', 'pipe'] });
