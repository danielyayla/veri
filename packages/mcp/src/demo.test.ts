...
// veri/ including its own workflow.md (DEC-018). This is the WO-004
// acceptance test — the demo package must match the mockup's context panel.
...
test('get_context("WO-002") on the demo matches the mockup context panel', async () => {
  ...
  assert.match(pkg.text, /## Workflow · WF-001 — skiff project workflow/);
  assert.match(pkg.text, /How work moves through skiff/);
