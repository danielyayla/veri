...
- 2026-09-02 — e2e36e0 — [".github/workflows/release.yml", "RELEASING.md"] — claude session fable-wo165 added the "Attach the Electron-bridge assets" step to the publish job (between upload and the completeness check): fetches the three files from the immutable v0.1.8 release, gates on the sha512 pinned inside the fetched latest-mac.yml, uploads with --clobber, and appends the names to ASSETS.txt so "Verify one complete release" refuses a release without them; RELEASING.md step 6 rewritten as verification-only. Evidence: release.yml parses (yaml pkg, step order confirmed); the gate's exact commands ran against the real v0.1.8 assets — pinned /kT3…Btsw== equals the computed digest (exit 0), and a single corrupted byte flips it to mismatch; non-app tags never reach the step (publish job already gated on guard.proceed). Finding: the v0.1.8 origin set (releaseDate 10:34:50) differs from the copy-forward chain shipped since (10:27:00) — two 0.1.8 builds existed; CI now heals the chain to the origin. The app-tag acceptance box stays open until the next real release run proves it; a scratch draft-release rehearsal was denied by session permissions.
- 2026-09-02 — verified — no code changes — ["v0.6.0 GitHub release assets"] — claude session fable-wo165, on the user's instruction, replaced v0.6.0's bridge assets with the v0.1.8 origin set (gh release upload --clobber): the live release now serves the 10:34:50 build — zip 207581206 bytes, latest-mac.yml pinning /kT3…Btsw== — verified by re-fetching latest-mac.yml from the release URL. The copy-forward divergence the receipt above found is healed on the current release, not just future ones.
