...
- 2026-08-18 — 395a285 + 4e71f65 — packages/core/src (drift.ts, approve.ts, +tests), packages/cli/src/commands.test.ts, veri/decisions/DEC-041 — drift's named remedy made real: a re-approval newer than the offending edit resolves edited-after-done (same anchor rule as [[DEC-041]]'s stamp detector), veri approve re-stamps already-promoted documents in place, and lifecycle ratification anchors on any commit — a byte-no-op re-approval still counts; DEC-041's rejected-alternatives section restored after the MCP writeback dropped it; Daniel re-approved the nine drifted documents and the corpus is drift-clean at 9 advisories, all historical receipt findings; 338 tests pass (agent session, Claude Code)
