...
- [ ] A fixture document with a 4-digit id parses, links (frontmatter and inline, both directions), renders as chips in reader and editor, and appears in search
- [ ] `veri/ids` with a 4-digit line parses, is honored as the floor, and survives a `recordIssuedId` rewrite intact
- [ ] Allocation proceeds 999 → 1000 without error; `padStart` output passes `ID_RE`
- [ ] All id-ordered lists sort `WO-999` before `WO-1000` (numeric, not lexicographic); package assembly order is byte-identical for existing all-3-digit projects
- [ ] Palette query `req14` does not match a 4-digit `REQ-1004`
