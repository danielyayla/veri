...
- [ ] An existing 0.1.x Electron install reaches the new version through the documented bridge path, and a subsequent tauri-updater update installs on quit — bridge half verified live 2026-08-21 (0.1.7 → differential update → 0.1.8 installed on quit → bridge offered the 0.2.0 installer against the published feed); the tauri-updater N→N+1 half waits for the first 0.2.x follow-up release, as the feed endpoint is fixed in tauri.conf.json
