...
- [x] An existing 0.1.x Electron install reaches the new version through the documented bridge path, and a subsequent tauri-updater update installs on quit — bridge half verified live 2026-08-21 (0.1.7 → differential update → 0.1.8 installed on quit → bridge offered the 0.2.0 installer against the published feed); tauri half verified same day against the published v0.2.1: the installed 0.2.0 logged "downloading 0.2.1" then "0.2.1 staged; takes effect at next launch", the on-disk app read 0.2.1 while 0.2.0 kept running (install-on-quit by construction, DEC-065), and the relaunch logged "app 0.2.1 launched"
...
- 2026-08-21 — 6e44f2a — veri/decisions/DEC-064…066 (stamps), veri/workflow.md (re-stamp), veri/work-orders/WO-073, WO-072 (notes) — Orchestrator pass under Daniel's delegation: DEC-064/065/066 and WF-001 approved, installed app independently re-verified (harness render of live 200-doc project; spike-vs-production focus control test — no divergence), spike-parity box checked; remaining unchecked box needs published releases and CI signing secrets only Daniel can add.
- 2026-08-21 — 271cefc — veri/work-orders/WO-073-migrate-the-desktop-shell-from-electron-to-tauri-2-node-side.md — Release day. Daniel added TAURI_SIGNING_PRIVATE_KEY(_PASSWORD) secrets and pushed main + both tags. v0.1.8's first run failed CI's zero-advisory self-test: the tag pointed at 5212466, which predates b5874ec's removal of WO-073's resolved DEC-008 link; fixed by re-tagging at 271cefc (5212466 plus that doc fix only, full suite green in UTC; merged into main so the tag commit is reachable). Both workflows produced drafts; before publishing, v0.1.8's latest-mac.yml + zip + blockmap were carried into the v0.2.0 release so pre-bridge Electron installs still resolve 0.1.8 after v0.2.0 became latest (electron-updater reads the latest release's feed file). Daniel published v0.1.8 then v0.2.0 (--latest). Live bridge walk verified: a fresh 0.1.7 install found 0.1.8 via the carried feed, differential-downloaded 44%, installed on quit, relaunched as 0.1.8, and the bridge logged "tauri feed advertises 0.2.0; offering the new installer". latest.json serves both architectures with valid minisign signatures. Only the tauri-updater N→N+1 install-on-quit clause remains, blocked until a 0.2.x follow-up release exists.
- 2026-08-21 — c126eaa — packages/ui/package.json, packages/ui/src-tauri/Cargo.toml, packages/ui/src-tauri/Cargo.lock — v0.2.1 update-path verification release (version bump only; precedent: v0.1.7 for WO-034). CI built, signed, and notarized the draft; the Electron bridge assets (latest-mac.yml + 0.1.8 zip + blockmap) were carried into it so 0.1.x stragglers keep resolving the bridge; Daniel published it as latest. Live tauri-updater walk on the installed 0.2.0: found and downloaded 0.2.1 from the published feed (minisign signature verified by the plugin), staged it — on-disk app read 0.2.1 while 0.2.0 kept running — and the relaunch logged "app 0.2.1 launched". Last acceptance box checked; WO-073 done. Note for future releases: carrying latest-mac.yml + the 0.1.8 zip forward is a manual release step until 0.1.x installs are believed extinct or it is automated in the release workflow.
