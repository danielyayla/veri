...
- [x] Clicking the Work Orders collection toggles its type panel like every other collection; the panel's top `▤ Board` row opens the ▤ Board tab and closes the panel — visual pass via the VERI_UI_SHOT harness: collection click opens the panel with the ▤ Board row atop (active-tinted, `view` hint) above the status groups; clicking the row lands on the ▦ Board tab with the panel closed
...
- [x] Board survives a 320px split pane via the stacked fallback — verified in a real split (document opened beside via ⌘\, divider at the 320px clamp): the board pane renders the single-column stacked layout, cards legible, no horizontal overflow
- [x] `veri check` green

## Receipts

- 2026-08-26 — 0ae6fef verified — no code changes — visual pass completed by a successor session (the claiming session's dev shell had gone stale): both deferred boxes verified against the current build via the VERI_UI_SHOT harness driving the real UI (panel entry with the ▤ Board row, board opened from the row with the panel closing, and the stacked fallback in a genuine 320px split pane). All acceptance boxes now checked; status set to done.
