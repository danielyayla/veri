...
- 2026-08-07 — packages/ui/src/main.ts, packages/ui/src/lib/root.ts, packages/ui/src/lib/root.test.ts — claude-code agent session (commit 11e9895, which predates the WO-nnn: subject convention): bare `npm start -w @veri/ui` now works — project root resolved by walking up from cwd to the nearest veri/ directory (explicit argument still wins); 3 new tests
