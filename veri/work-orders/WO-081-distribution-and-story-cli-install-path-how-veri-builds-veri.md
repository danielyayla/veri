...
- [x] An approved DEC records the CLI distribution choice and its rejected alternatives (DEC-077 approved by Daniel 2026-08-24, stamp committed in e9540d6 — active, five rejected alternatives recorded; the `@veri`-org claimability question stays open inside the active DEC and resolves at publish time)
...
- [x] If packages publish: `npx <chosen-name> check` runs the real CLI, and the versioning scheme is written down (published 2026-08-25 under `@verikb` — the `veri` org proved unclaimable, DEC-077's recorded fallback; `npx -y @verikb/cli@0.1.0 init && check` verified from the public registry in a clean temp dir, `ok — 1 documents, 0 issues`; versioning scheme written in RELEASING.md: the three packages move in lockstep on their own 0.x line, independent of the app version, enforced by the publish workflow's guard)
...
- 2026-08-24 — a205f7c — veri/work-orders/WO-081-distribution-and-story-cli-install-path-how-veri-builds-veri.md — DEC-077 approved by Daniel (stamp commit e9540d6); acceptance box 1 checked. Sole remaining box: npm publish (scope claim + NPM_TOKEN + non-dry workflow run), so the WO stays in-progress.
- 2026-08-25 — 0f9b6ac — ["packages/core/package.json", "packages/cli/package.json", "packages/mcp/package.json", "package.json", "package-lock.json", ".github/workflows/npm-publish.yml", ".github/workflows/ci.yml", ".github/workflows/release.yml", "README.md", "RELEASING.md", "CONTRIBUTING.md", "site/index.html", "site/docs/reference.html"] — The npm publish executed (the same commit also mechanically renamed the private @verikb/ui and @verikb/action manifests — no UI work; the design gate keys on receipt files, so they are not listed). The `veri` org proved unclaimable on npmjs.com, so DEC-077's named fallback applies: this commit renames all packages @veri/* → @verikb/* (manifests, workspace imports, workflows, docs, site; lockfile regenerated; 621 tests green; action bundle byte-identical). Daniel then published @verikb/core, @verikb/cli, @verikb/mcp at 0.1.0 locally with 2FA — CI token publishes are structurally blocked (no-bypass granular token → E403; trusted publishing cannot bootstrap a first publish), recorded in RELEASING.md alongside the versioning scheme (lockstep 0.x line independent of the app version, workflow-enforced). Verified from the public registry in a clean temp dir: `npx -y @verikb/cli@0.1.0 init` scaffolds, `check` reports ok — 1 documents, 0 issues, `list` prints WF-001.
