...
2. **The worth-making trace.** A work order is admissible as `ready` only if it traces — directly or transitively through its links — to a live (non-withdrawn, non-retired) requirement. A ready or in-progress work order with no such trace is orphan execution and fails `veri check`. This enforces the redefinition of a work order as *the smallest bounded product change we are currently confident is worth making*, rather than a ticket naming files to modify. Backlog work orders are exempt: sketching is free; readiness is the gate.
...
- [ ] A `ready` or `in-progress` work order with no direct or transitive link to a live requirement fails `veri check`
